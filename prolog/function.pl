...
extract_fntype_arg_([], []) :- !.
extract_fntype_arg_([['uast:FunctionType', Args, _, _] | TailList], JoinedList) :- \+ var(Args), extract_fntype_arg_(TailList, List), join_(Args, List, JoinedList), !.
...
% function_def extracts function name and function arguments for each function_group.
